...
public class CorridorFirstDungeonGenerator : AbstractDungeonGenerator
{
    ...
    [SerializeField] protected bool spawnDoorAtRandom = true; // Toggle for random placement or near player
     ...
     public float enemyDistanceChangeType = 10f;
    ...
    private Vector2Int doorPositionJoobEdition;
    ...
    protected void PlaceDoorAtRandomFloor(HashSet<Vector2Int> floorPositions)
    {
        ...
        doorPositionJoobEdition = doorPosition;
    ...
    protected void SetupEnemySpawner(HashSet<Vector2Int> floorPositions)
    {
        ...
        if (TrySpawnBossNearDoor(doorPositionJoobEdition, floorPositions, out Vector3 bossSpawnPosition))
        {
            spawnerScript.SpawnBoss(bossSpawnPosition);
        }
        else
        {
            Debug.LogWarning("Failed to find a valid floor position for the boss near the door.");
        }
    }

    private bool TrySpawnBossNearDoor(Vector2Int doorTilePosition, HashSet<Vector2Int> floorPositions, out Vector3 bossSpawnPosition)
    {
        List<Vector2Int> adjacentPositions = new List<Vector2Int>
    {
        doorTilePosition + Vector2Int.up,
        doorTilePosition + Vector2Int.down,
        doorTilePosition + Vector2Int.left,
        doorTilePosition + Vector2Int.right
    };
        ...
        foreach (var position in adjacentPositions)
        {
            ...
            {
                bossSpawnPosition = new Vector3(position.x + 0.5f, position.y + 0.5f, 0);
                return true;
            }
        }
